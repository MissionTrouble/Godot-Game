...
func _ready():
		...
		$KEY.text = OS.get_scancode_string(Save.controls["move_left"][index])
		updateArrows()
...
func updateArrows():
	if $KEY.text == OS.get_scancode_string(Save.controls["move_left"][0]):
		$LeftArrow.visible = false
	else:
		$LeftArrow.visible = true
	if $KEY.text == OS.get_scancode_string(Save.controls["move_left"][Save.controls["move_left"].size()-1]):
...
#		el
		if add:
			...
			Save.controls["move_left"][index] = event.scancode
			get_parent().get_parent().get_parent().get_parent().get_parent().get_parent().get_node("KeyPopup").visible = false
			changing = false
...
func _on_DELETE_pressed():
	if Save.controls["move_left"].size() >1:
		Save.controls["move_left"].remove(index)
		if index >= Save.controls["move_left"].size():
			index -= 1
			$KEY.text = OS.get_scancode_string(Save.controls["move_left"][index])
		elif index >= 0:
			$KEY.text = OS.get_scancode_string(Save.controls["move_left"][index])
